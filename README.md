...
- [Nuvo Serial](#nuvo-serial)
  ...
  - [Services](#services)
    - [nuvo_serial.page_on](#nuvo_serial.page_on)
    - [nuvo_serial.page_off](#nuvo_serial.page_off)
    - [nuvo_serial.party_on](#nuvo_serial.party_on)
    - [nuvo_serial.party_off](#nuvo_serial.party_off)
    - [nuvo_serial.snapshot](#nuvo_serial.snapshot)
    - [nuvo_serial.restore](#nuvo_serial.restore)
    - [nuvo_serial.configure_time](#nuvo_serial.configure_time)
  - [Keypad Services](#keypad-services)
    - [nuvo_serial.simulate_next_button](#nuvo_serial.simulate_next_button)
    - [nuvo_serial.simulate_prev_button](#nuvo_serial.simulate_prev_button)
    - [nuvo_serial.simulate_play_pause_button](#nuvo_serial.simulate_play_pause_button)
  - [Keypad now-playing metadata](#keypad-now-playing-metadata)
    - [nuvo_serial.set_source_display](#nuvo_serial.set_source_display)
    - [nuvo_serial.set_source_track](#nuvo_serial.set_source_track)
  - [Automations Triggers](#automations-triggers)
  - [Zone Groups](#zone-groups)
    - [HA service call example](#ha-service-call-example)
    - [Mini media player group control](#mini-media-player-group-control)
...
Queries the state of a Nuvo Grand Concerto/Essentia G amplifier and exposes its
configuration as Home Assistant entities.
...
Connection to the Nuvo amplifier is via the "Programming and Serial Control"
RS-232 9-pin female connector, either cabled direct to the device running Home
Assistant or remotely over a network using a serial-to-network device. The
integration use the [serialx](https://puddly.github.io/serialx/) library so any
of its supported backends and URI schemes will work.

### Direct

Direct connection to the device running Home Assistant, either through a
serial-to-serial cable if the device has a serial port, or a USB-to-RS-232
9-pin male cable.
...
[ESPHome serial
proxy](https://www.home-assistant.io/blog/2026/05/06/release-20265/#serial-ports-over-the-network-with-esphome)

#### ser2net

Connecting to the Nuvo over a network is possible using a hardware
serial-to-network adapter or software such as
...
### nuvo_serial.configure_time

Set the real time clock in the Grand Concerto.  The Essentia has no real time clock.
...
## Keypad now-playing metadata

### nuvo_serial.set_source_display

This command will set the display line information for a source. This command
can only be used with non-NuVoNet sources.

### nuvo_serial.set_source_track

This command will set a sources display track status information for a source.
This command can only be used for non-NuVoNet sources.

With these two service it's possible to use HA automations to [push metadata
from HA media players to Nuvo keypads](https://github.com/sprocket-9/hacs-nuvo-serial/wiki/Displaying-Now%E2%80%90Playing-Metadata-on-Zone-Keypads).
...
An [example configuration ](https://github.com/sprocket-9/hacs-nuvo-serial/wiki/Example-frontend-configuration-in-yaml-using-Mini-Media-Player)
written in yaml and using Mini-media player.

## Debugging

Go to Settings/Devices & Services/Nuvo integration and turn on "Enable debug
logging" from the burger menu.  Check the log for error messages.  If submitting
an issue **attach** the **full** debug log showing the issue.

## Known Issues

It's fairly common for new users of the integration to get a
"Failed To Connect" message and python exceptions showing
in the debug log, often concerning SourceMask values:
...
If this is the case, the Nuvo system unit contains an invalid configuration and
requires using the Nuvo Windows Configuration software to reset the configuration.
Save the current configuration then reupload it to the unit before trying the
integration again.  If that doesn't work then upload the Default Configuration
